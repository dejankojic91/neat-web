...
const CookieOptions = () => {
    ...
    return (
        <div className="pl-4 pt-3 space-y-4 border-l border-muted text-sm">
            <RadioGroup value={mode} onValueChange={(v) => setMode(v as "whitelist" | "blacklist")} className="flex gap-4">
            ...
            <div className="space-y-1">
                {domains.length === 0 ? (
                    <p className="text-muted-foreground text-xs italic">No filters defined</p>
                ) : (
                    domains.map((domain, idx) => (
                        <div key={idx} className="flex justify-between items-center bg-muted px-2 py-1 rounded-md">
